...
public class AttackPhase {
    ...
    private int waveCompletionReward = 10;
    ...
    public void runWave(Map map, Player player, List<Tower> towers, Wave wave, int waveNumber) {
        ...
        while (!pendingEnemies.isEmpty() || !activeEnemies.isEmpty()) {
            ...
            allTowersPerformAttack(towers, player, activeEnemies, path);
    ...
    private void allTowersPerformAttack(List<Tower> towers, Player player, List<Enemy> activeEnemies, List<Coordinate> path){
        for (Tower t : towers) {
            t.tickCooldown();
            if(t.canAttack()){
                t.attack(player, activeEnemies, path);
            }
        }
    }
    ...
    public int getWaveCompletionReward() {
        return waveCompletionReward;
    }
}
