...
public class BuildPhase {
    ...
    public void selectTower(Map map, Cursor cursor, Player player){
        ...
        Character choice = playerInput.readTowerSelection();
        if (choice == null) return;

        Tower t = createTowerFromChoice(choice, cursor);
    ...
    private Tower createTowerFromChoice(char choice, Cursor cursor) {
        ...
        return switch (choice) {
            case '1' -> new Tower_weak(x, y);
            case '2' -> new Tower_medium(x, y);
            case '3' -> new Tower_strong(x, y);
            default -> null;
        };
    }
